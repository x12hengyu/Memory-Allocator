// YOUR PREFERED NAME: XIZHENG YU
// WISC NETID: XYU354
// CANVAS USERNAME: XIZHENG YU
// WISC ID NUMBER: 9082566994
// SUBMISSION DATE (MADISON TIME ZONE): AUGEST 9 2021
// COMMENTS FOR THE GRADER (OPTIONAL): NONE
 ...
 *  10) Set_Size 
 */
BLOCK_HEADER *Get_Next_Header(BLOCK_HEADER *head) {
    if (head->packed_pointer == NULL) {
        return NULL;
    } else {
        return (BLOCK_HEADER *)((unsigned)head->packed_pointer & ~1);
    }
}
int Is_Free(BLOCK_HEADER *head) {
    if (((unsigned)(head->packed_pointer) & 1) == 1)
        return 0;
    return 1;
}
    
int Get_Size(BLOCK_HEADER *head) {
    return head->size;
}
...
void* Mem_Alloc(int size) {
    ...
    if (size < 4) {
        ...
        align_size = (size / 4) * 4 + 4;
    }
    // find available space
    BLOCK_HEADER *cur = first_header;
    while(cur != NULL) {
        if(Is_Free(cur) == 1 && Get_Size(cur) >= align_size) {
            break;
        } else {
            cur = (BLOCK_HEADER*)((unsigned)cur->packed_pointer&0xfffffffe);
    ...
    // large would need break
    if (Get_Size(cur) > align_size) {
        // address of next pointer
        BLOCK_HEADER *point_to = cur->packed_pointer;
        cur->packed_pointer = (void *)cur + align_size + sizeof(BLOCK_HEADER);
        // next free space pointer
        BLOCK_HEADER *next = (BLOCK_HEADER *)cur->packed_pointer;
        next->size = cur->size - align_size - sizeof(BLOCK_HEADER);
        next->packed_pointer = point_to; // point to end
        // change current allocated size
        cur->size = align_size;
    }
    // mark last bit 1
    cur->packed_pointer = (void *)((unsigned)cur->packed_pointer | 1);
...
int Mem_Free(void *ptr)
{
    ...
    // check all the block headers
    BLOCK_HEADER *cur = first_header;
    while (cur != (ptr-(sizeof(BLOCK_HEADER)))) {
        cur = Get_Next_Header(cur);
        if (cur == NULL) {
            return -1;
        }
    }
    // pointing to a not allocated block
    if (Is_Free(cur)) {
        return -1;
    }
    ...
    while (cur->packed_pointer != NULL) {
        if (cur != (ptr-(sizeof(BLOCK_HEADER)))) {
            prev = cur;
            cur = Get_Next_Header(cur);
        } else {
            break;
    ...
    cur->packed_pointer = (void *)((unsigned)cur->packed_pointer & ~1);
    // check next empty
    if (cur->packed_pointer != NULL && Is_Free(cur->packed_pointer)) {
        BLOCK_HEADER *next = cur->packed_pointer;
        cur->packed_pointer = next->packed_pointer;
        cur->size = cur->size + next->size + sizeof(BLOCK_HEADER);
        // clear next
    }
    if (prev != NULL && Is_Free(prev)) {
        prev->packed_pointer = cur->packed_pointer;
        prev->size = cur->size + prev->size + sizeof(BLOCK_HEADER);
    }
    return 0;
}
